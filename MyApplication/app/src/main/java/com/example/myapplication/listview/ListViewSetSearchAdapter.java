...
public class ListViewSetSearchAdapter extends RecyclerView.Adapter<ListViewSetSearchAdapter.CustomViewHolder>{

    private ArrayList<ListViewSetItem> listViewItemList = new ArrayList<ListViewSetItem>() ;
    private FragmentSearchSet.Command command;

    //생성자에서 데이터 리스트 객체를 전달받음
    public ListViewSetSearchAdapter(ArrayList<ListViewSetItem> list, FragmentSearchSet.Command com) {
        listViewItemList = list;
        command = com;
    }
    //아이템 뷰를 위한 뷰홀더 객체 생성하여 리턴
    @Override
    public ListViewSetSearchAdapter.CustomViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        Context context = parent.getContext();
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);

        View view = inflater.inflate(R.layout.lisetview_setitem, parent, false);
        ListViewSetSearchAdapter.CustomViewHolder viewHolder = new ListViewSetSearchAdapter.CustomViewHolder(view);
        return viewHolder;
    }
    //position에 해당하는 데이터를 뷰홀더의 아이템뷰에 표시
    @Override
    public void onBindViewHolder(ListViewSetSearchAdapter.CustomViewHolder holder, int position) {
        ...
        public CustomViewHolder(View itemView) {
            ...
            itemView.setOnClickListener(new View.OnClickListener(){
                ...
                public void onClick(View v){
                    ...
                    command.searchSetClicked(set_no, (String)set_name.getText(),
