...
public class GameOneActivity extends AppCompatActivity {
    ...
    private ArrayList<ListViewSetWordItem> Copyitemlist = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String set_no = intent.getStringExtra("set_no");
        ArrayList<ListViewSetWordItem> itemlist = (ArrayList<ListViewSetWordItem>)intent.getSerializableExtra("itemlist");

        Log.d("TAG", "set_no is "+set_no);
        ...
        try {
            ...
            for(int i=0; i<jarray.length(); i++){
                ...
                //카드 앞면
                Log.d("ImgBit", i+" ImgBit is "+ itemlist.get(i).getImgBit());
                Log.d("hint", "hint is "+ jsonObject.getString("hint"));
                adapter.addItem(jsonObject.getString("word_a"), itemlist.get(i).getImgBit(),
                        jsonObject.getString("hint"));
                adapter.notifyDataSetChanged();
                //카드 뒷면 저장
                gameOneItem.setCardText(jsonObject.getString("word_b"));
                BackList.add(gameOneItem);
            }
